# 長庚大學自動查詢郵件 v1.2.2｜CGU Postal Mail Checker v1.2.2
...
- 「產生診斷 JSON」會輸出外掛版本、Chrome 版本、查詢欄位狀態、錯誤紀錄及最近查詢結果。下載完成後會顯示 `weihao.chiu@gmail.com`，並詢問是否開啟系統預設郵件程式；請自行附加診斷檔案。
- 「產生診斷 ZIP」包含相同 JSON；如果已啟用錯誤截圖且曾擷取成功，也會加入最近一次畫面。
- 錯誤截圖預設關閉。啟用後，長庚查詢頁只會在查詢錯誤時於本機建立畫面，不使用 Chrome 除錯權限；關閉後會清除暫存截圖。
